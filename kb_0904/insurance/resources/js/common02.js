$(document).ready(function(){
	...
	$(".accordion_wrap").each(function() {
		...
		if($(this).hasClass("m-type-diff")) {
			$(this).find(".accordionheader").each(function() {
				$(this).find(".detail").detach();
				$(this).append("<span class='detail'>자세히보기</span>");
			});
		}

		$(this).find(".accordionheader").on("click", function(event, param=null) {
			if($(this).closest(".accordion_wrap").hasClass("m-type-diff") && $(window).width()<=768) {
				$(this).next().show().siblings(".accordionbody").hide();
				if(param==null) $(this).next().scrollTop(0);
				$(this).next().find(".m-title").detach();
				$(this).next().prepend("<h3 class='m-title'>" + $(this).find("h3").text().replace("자세히보기","") + "<a href='#'>Close</a></h3>")
				$(this).next().css("paddingTop",$(this).next().find(".m-title").outerHeight()+20);
				$(this).next().find(".m-title > a").on("click", function() {
					$(this).closest('.accordionbody').hide();
					return false;
				});

				checkTh();
				tableNoData();
				if(param) paranScroll(param, $(this).next());
	...
	function paranScroll(param, m=null) {
		setTimeout(function() {
...
$(window).on("resize", function() {
	...
	if(screenX<=768) {
		...
		$(".accordion_wrap.m-type-diff .accordionheader").removeClass("on");		
		$(".accordion_wrap.m-type-diff .accordionbody").hide();
		checkTh();
		tableNoData();
	} else {
		...
		$(".accordion_wrap.m-type-diff .accordionheader").removeClass("on");
		$(".accordion_wrap.m-type-diff .accordionbody").removeAttr("style").hide();
		checkTh();
		tableNoData();
	}
});
...
function motionCheck(tabgroup) {
	if($(tabgroup).find(".accordion_wrap").length>0) {
		if(!$(tabgroup).find(".accordion_wrap").find(".accordionheader").eq(0).hasClass("on") && screenX>768) {
	...
	$("[href='#rateMap']").on("click", function() {
		...
		$("#rateMap .rate_table_title").each(function(i) {
			...
			if(compare_text == lnk_text) {
				$(".accordion_wrap .accordionheader").eq(4).trigger("click", i+1);
			}
